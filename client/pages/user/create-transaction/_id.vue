...
export default {
  ...
  async fetch() {
    ...
    const userTransaction = (
      await this.$api.get(`/transactions/${transid}?access_token=${id}`)
    ).data;

    this.customer_name = userTransaction.customer_name;
    this.amount = userTransaction.amount;
    this.description = userTransaction.description;
  ...
  methods: {
    async updateTransaction(e) {
      ...
      let count = 0;
      if (this.customer_name.toString().trim().length == 0) {
        count++;
        this.errors.customer_name = "Customer name is Required";
      }
      // debugger;

      if (this.amount.toString().trim().length == 0) {
        count++;
      ...
      if (this.type.toString().trim().length == 0) {
        count++;
      ...
      const transid = this.$route.params.id;
      const userTrans = (
        await this.$api.patch(`/transactions/${transid}?access_token=${id}`, {
